...
public final class KeyInput extends Input {
	...
	private List<KeyControllable> controllers;
	
	public KeyInput(Engine engine) {
		super(engine, "acl.io.keyboard", "acl_internal_keyboard");
	...
	public void onUpdate() {
		if (Keyboard.getEventKey() != 0 && Keyboard.getEventKeyState()) {
			...
			controllers.forEach(controller -> controller.onKeyHold(key));
		}
		
		while (Keyboard.next()) {
			int key = Keyboard.getEventKey();
			if (Keyboard.getEventKeyState() && !Keyboard.isRepeatEvent())
				controllers.forEach(controller -> controller.onKeyPress(key));
			else if (!Keyboard.isRepeatEvent())
				controllers.forEach(controller -> controller.onKeyRelease(key));
		}
	}
	
	public void addController(KeyControllable controller) {
		controllers.add(controller);
	}
	
	public void removeController(KeyControllable controller) {
